...
parser.add_argument("--update_ado", default=False, type=str, help="ADO Status update required or not")
...
if (args.mode == "testcase"  or args.mode =="testsuite"):
    temp_file_name = 'Scriptless Execution.' + args.name   #it will be replaced using Bilint
    CM.create_temp_suite_config_file(temp_file_name)
    CM.set_engagement_information_in_suite_config(temp_file_name,"Primary","engagementId", engagementid_primary)
    CM.set_engagement_information_in_suite_config(temp_file_name,"Component","engagementId", engagementid_component)
    CM.set_engagement_information_in_suite_config(temp_file_name,"Archive","engagementId", engagementid_archive)
...
if args.update_ado:
    init_run.update_ado_status(args)
CM._delete_temp_files('Scriptless Execution.')
sys.exit(1) if status > 0 else sys.exit(0)
